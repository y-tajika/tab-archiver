...
**現在のバージョン**: v3.0.0（LLM要約機能を実装）

### 主要機能
1. 全タブ一括ブックマーク
2. 自動フォルダ名生成（デフォルト形式: `月-日_要約キーワード`）
3. 手動フォルダ名編集（ポップアップUI・インライン編集）
4. キーボードショートカット
5. カスタムブックマークフォルダ選択
6. タブ一覧表示・編集（削除・除外）
7. 保存時に保存先フォルダを一時的に選択（v2.6.0）
8. フォルダツリー折りたたみ・リネーム機能（v2.7.0）
9. LLMによるタブ要約（v3.0.0）
...
tab-archiver/
  tab-archiver-extension/    # 現在の開発バージョン
    src/                   # Chromeにロード
       ...
       options.html/css/js
       constants.js        # 共通定数
       folder-utils.js     # フォルダ操作共通モジュール (v2.7.0)
       icons/
    README.md
    CHANGELOG.md
    DEVELOPMENT_LOG.md
...
**設計方針:**
- ルールベース要約を使わず、LLMで40文字以内の要約を生成
- 生成はユーザー操作（✨ボタン）でオンデマンド
- 連打抑止として2秒クールダウン

**実装詳細:**
- 対象タブ: 最大10件のタイトルを使用
- OpenRouter/Gemini 両対応
- 出力クリーニング: 改行/末尾記号の除去、40文字に切り詰め
- 重複対策: 同一行の除去と連続重複の圧縮

**外部設定ファイル（開発用）:**
- `tab-archiver-extension/src/ai-config.json`
- 形式: `{ "provider": "openrouter|gemini", "apiKey": "..." }`
- Git管理から除外（.gitignoreに追加）

**OpenRouterモデル指定:**
- 既定は `tngtech/deepseek-r1t-chimera:free`
- ルーティングやモデル制御はOpenRouter側の設定と併用可能
