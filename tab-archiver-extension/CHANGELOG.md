...
**背景:** ルールベース要約を廃止し、LLM要約へ移行

### Added
- LLM要約ボタン（✨）をポップアップに追加
- OpenRouter / Gemini によるAI要約を実装
- 外部設定ファイル `ai-config.json` の自動読み込み
- 外部設定がある場合はAI要約を自動有効化

### Changed
- 要約長を 15→40 文字に拡張
- LLMは最大10タブのタイトルから要約を生成
- OpenRouterで無料モデル `tngtech/deepseek-r1t-chimera:free` を使用
- 要約生成時の重複出力を除去
...
### Removed
- LLM要約キャッシュ（毎回再生成する方針に変更）

## [2.7.0] - 2026-01-16

**背景:** フォルダ操作の使い勝手を大幅に改善

### Added
- フォルダ名編集機能を追加
  - F2キーまたはダブルクリックでフォルダ名を直接編集可能
  - Enterで確定、Escapeでキャンセル
- フォルダツリーの折りたたみ機能を実装
  - ▶/▼アイコンでフォルダを開閉可能
  - デフォルトで全フォルダが閉じた状態、必要なものだけ展開
  - 新規フォルダ作成時に展開状態を維持
- フォルダ操作機能をfolder-utils.jsに共通化
  - options.jsとpopup.jsでコードを共有、保守性向上

### Changed
- 新しいフォルダ作成時のpromptを削除
  - デフォルト名「新しいフォルダ」で即座に作成
  - 作成後自動的にリネームモードに入り、スムーズな名前変更が可能
- ボタンテキストを簡潔化
  - 「新しいフォルダ」→「新規」にUI最適化

### Improved
- フォルダツリーの視認性を大幅に改善
- モーダルUIのサイズとボタン配置を最適化
