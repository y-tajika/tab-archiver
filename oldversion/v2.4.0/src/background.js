...
async function saveAllTabs(customFolderName = null, excludedTabIds = []) {
  try {
    const settings = await getSettings();
    
    // ウィンドウ取得モードに応じて処理を分岐
    if (settings.windowSaveMode === 'all-windows') {
      return await saveAllTabsAllWindows(customFolderName, settings, excludedTabIds);
    } else {
      return await saveAllTabsCurrentWindow(customFolderName, settings, excludedTabIds);
    }
  } catch (error) {
 ...
 */
async function saveAllTabsCurrentWindow(customFolderName, settings, excludedTabIds = []) {
 ...
 */
async function saveAllTabsAllWindows(customFolderName, settings, excludedTabIds = []) {
  ...
  const tabLoggerFolderId = await ensureTabLoggerFolder(settings);
  const mainFolderId = await createFolder(tabLoggerFolderId, mainFolderName);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'saveWithCustomName') {
    ...
    saveAllTabs(request.folderName, excludedTabIds).then(result => {
      sendResponse(result);
    });
    return true; // 非同期レスポンス
  }
