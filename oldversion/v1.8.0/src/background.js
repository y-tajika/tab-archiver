...
function findOtherBookmarks(nodes) {
  ...
  for (const title of titles) {
    ...
    if (id) {
      console.log('Tab Logger: その他のブックマーク発見:', id, title);
      return id;
    }
  }
  return null;
...
async function ensureTabLoggerFolder(settings) {
  try {
    if (settings.useCustomFolder && settings.targetFolderId) {
      console.log('Tab Logger: カスタムフォルダを使用:', settings.targetFolderId);
    ...
    const folderId = searchFolder(tree, 'Tab_Logger');
    if (folderId) {
    ...
    const newFolder = await createFolder(otherBookmarksId, 'Tab_Logger');
    console.log('Tab Logger: Tab_Loggerフォルダ作成完了:', newFolder);
    return newFolder;
  } catch (error) {
    console.error('Tab Logger: フォルダ確保エラー:', error);
    throw error;
  }
}
...
async function createFolder(parentId, title) {
  try {
    ...
    console.error('Tab Logger: フォルダ作成エラー:', error);
    throw error;
  }
}
...
async function createBookmark(parentId, title, url) {
  try {
    ...
    console.error('Tab Logger: ブックマーク作成エラー:', error);
    throw error;
  }
}
...
async function saveAllTabs(customFolderName = null) {
  try {
    ...
    if (failedCount > 0) {
      console.error(`Tab Logger: ブックマーク保存失敗: ${failedCount}件`);
    }
    ...
    console.error('Tab Logger: 保存処理エラー:', error);
